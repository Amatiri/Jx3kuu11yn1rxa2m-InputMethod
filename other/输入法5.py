        ...
        for line in f:
            ...
            if len(parts) == 2:
                ...
                if len(prefix) >= 5 and prefix[4] == 'a':
                    ...
                    elif len(prefix)==5 and code==prefix[:4]:#取a
                        rest = code[4:]
                        results.append(f"{parts[0]}{rest}")
...
def update_display():
    ...
    if processed and not skip_reset_flag:
    ...
    if current_query_type == "multi_part":
        ...
        if first_chars:
            if current_phrase and not in_part_selection:
                ...
                if first_chars and phrase_content:

                    if first_chars == phrase_content:
...
def handle_special_keys(input_text):
    global skip_reset_flag, current_phrase
...
def handle_selection_keys(event):
    ...
    if event.char == "!" and current_phrase:
        ...
        replace_content(input_text, phrase_content)
    ...
    if event.char in selection_symbols:
        ...
        if 0 <= index < len(candidates):
            ...
            if current_query_type == "single":
                processed = process_input(input_text)
                split_text = split_sequence(processed)
                replace_content(input_text,selected_char)
                ...
                if "'" in split_text:
                    ...
                    if current_part_index < len(parts):
                        ...
                        replace_content(input_text, new_processed)


                        has_next_part = current_part_index + 1 < len(parts)
                        
                        if has_next_part:
                            global skip_reset_flag
                            skip_reset_flag = True
                            
                            processed_new = process_input(new_processed)
                            split_text_new = split_sequence(processed_new)
                            if "'" in split_text_new:
                                current_split_parts = split_text_new.split("'")
                            
                            navigate_parts("next")
                            
                            first_chars = query_multi_chars(new_processed)
                            first_chars_label.config(text=first_chars if first_chars else '')
                            
                        else:
                            global in_part_selection
                            in_part_selection = False
                            pyperclip.copy(output)
                            first_chars = query_multi_chars(new_processed)
                            first_chars_label.config(text=first_chars if first_chars else '')
...
def main_function(*args):
    global current_page, current_part_index, current_query_type, current_split_parts, previous_input, in_part_selection, skip_reset_flag, current_phrase
    global xrze 
    input_text = real_time_var.get()
...
def initial(event):
    ...
    if filt == 2:
        filt = 1
        if event.name in "qwertyuiopasdfghjkl;zcxvbnm.'1234567890":
            # 将字符插入到输入框光标位置
            ...
            entry_box.icursor(cursor_pos + 1)  # 移动光标
            #real_time_var.set(new_text)  # 触发主函数
            total += 1
        elif event.name in ["-", "=", "!", "@", "#", "$", "%", "space", "up", "down", "left", "right", "backspace"]:
            if event.name == "-":
                # 切换到上一个编码部分
                navigate_parts("prev")
...
def start_keyboard_listener():
    ...
    keyboard.add_hotkey('shift+left', toggle, suppress=True)
...
def toggle_auto_commit():
    ...
    if auto_commit_var.get() == "1":
        xrze = "1"
        auto_commit_label.config(text="自动上屏", fg='#006600')
    else:
        xrze = ""
        auto_commit_label.config(text="自动上屏", fg='#990000')
auto_commit_label = tk.Label(settings_frame, text="自动上屏", 
...
def toggle_auto_commit_click(event):
    ...
    auto_commit_var.set(xrze)  # 保持与变量的同步
...
def toggle_phrase_commit():
    ...
    if phrase_commit_var.get() == "1":
        phrase_commit = "1"
        phrase_commit_label.config(text="词语优先", fg='#006600')
    else:
        phrase_commit = ""
        phrase_commit_label.config(text="词语优先", fg='#990000')
phrase_commit_label = tk.Label(settings_frame, text="词语优先", 
                               font=("楷体", 14), bg='#FFF3C7',
...
def toggle_phrase_commit_click(event):
    ...
    phrase_commit_var.set(phrase_commit)  # 保持与变量的同步
...
def create_radical_table():
    ...
    tk.Label(title_frame, text="部母", font=("华文中宋", 11), 
...
def toggle_radical_table():
    if radical_table_var.get():
        ...
        radical_table_label.config(text="部母表", fg='#006600')
        window.geometry("600x565")
        create_radical_table()
    else:
        radical_table_frame.pack_forget()
        radical_table_label.config(text="部母表", fg='#990000')
        window.geometry("600x215")
radical_table_label = tk.Label(settings_frame, text="部母表", 
                              font=("楷体", 14), bg='#FFF3C7', 
